...
# Backwards compatibility for existing admin-only views
...
@login_required
def notification_email_page():
    user = User.query.get(session['user_id'])
    return render_template('notification_email.html', user=user)
...
def models_page():
    user = User.query.get(session['user_id'])
    return render_template('models.html', user=user)


@app.route('/input_files')
@login_required
def input_files_page():
    user = User.query.get(session['user_id'])
    return render_template('input_files.html', user=user)
...
def init_db():
    ...
    with app.app_context():
        ...
        if not Role.query.filter_by(name='user').first():
            db.session.add(Role(name='user', description='Regular User', permissions=''))
        if not User.query.filter_by(principal_name='admin').first():
            admin_user = User(name='Admin', principal_name='admin', role='admin', email='admin@example.com', group_name='Default')
            admin_user.set_password('admin')
            db.session.add(admin_user)
        if not User.query.filter_by(principal_name='user1').first():
            regular_user = User(name='User One', principal_name='user1', role='user', email='user1@example.com', group_name='Default')
